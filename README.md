...
- ✅ PLAYING 装弹/拆弹：进攻方 slot 7 拿 TNT 矿车、防守方 slot 7 拿剪刀，在炸弹范围内
  右键开始，按 `plant_time` / `defuse_time` 读条完成；读条期间冻结移动（保留视角），
  再次右键取消；受伤/死亡/炸弹状态变化会打断；打断/取消后自动恢复移动；
  已安放（PLANTED）炸弹每秒在中心点生成红色灰尘粒子
- ✅ PLAYING 战斗菜单物品：slot 8 悬挂式橡木告示牌，在战斗/等待重生等状态下都保留；
  右键打开空箱子菜单（空实现，后续填充）
