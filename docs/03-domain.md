...
| `PlayerState` | 玩家状态：WAITING / ROLE_SELECTING / DEPLOYING / IN_BATTLE（DEPLOYING 预留给死亡部署流程） |
...
- `SectorManager`（单例）：
  ...
  - `cleanup()` 停掉所有引信/时限任务（FINISHED 调用）。
